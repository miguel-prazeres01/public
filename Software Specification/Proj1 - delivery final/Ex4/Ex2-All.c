...
int countDiff (int arr[], int size) {
  ...
  int ret = 1; 
  int prev = arr[0]; //BUG: Nullptr Dereference. Not detected by Infer

  for (int i=1; i<size; i++) {
    if (arr[i] != prev) {  //BUG: Nullptr Dereference. Not detected by Infer
      prev = arr[i];
      ret++;
    }
  }
...
ret_pair removeRepsArr(int arr[], int size) {
  ...
  ret_pair rp = (ret_pair)malloc(sizeof(struct ret_pair_struct));  //BUG: Nullptr Deference. Not detected by Infer.
  rp->fst = new_arr; 
  rp->snd = new_size;
