...
public class FailedAnswerService {
    ...
    public List<FailedAnswerDto> getFailedAnswers(int dashboardId){
        ...
        List<FailedAnswer> failedAnswerList =  dashboard.getFailedAnswers();
        List<FailedAnswerDto> failedAnswerDtoList = new ArrayList<FailedAnswerDto>();
        failedAnswerList.stream().forEachOrdered( failedAnswer -> {FailedAnswerDto failedAnswerDto = new FailedAnswerDto(failedAnswer);failedAnswerDtoList.add(failedAnswerDto);});
        Collections.sort(failedAnswerDtoList,(f1, f2) -> f2.getQuestionAnswerDto().getQuestion().getId()- f1.getQuestionAnswerDto().getQuestion().getId());

        return failedAnswerDtoList;
    }
