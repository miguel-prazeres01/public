...
public class    TutorPermissionEvaluator implements PermissionEvaluator {
        ...
        if (targetDomainObject instanceof Integer) {
            ...
            switch (permissionValue) {
                ...
                case "WEEKLYSCORE.ACCESS":
                    return userHasThisWeeklyScore(authUser,id);
                ...
                case "DIFFICULT_QUESTION.ACCESS":
                    DifficultQuestion dQuestion = difficultQuestionRepository.findById(id).orElse(null);
                    return dQuestion != null && accessDifficultQuestion(authUser, dQuestion);
    ...
    private boolean userHasThisDashboard(AuthUser authUser, int dashboardId){
        Dashboard dashboard = dashboardRepository.findById(dashboardId)
                .orElse(null);
        return dashboard!=null && dashboard.getStudent().getId().equals(authUser.getUser().getId());
    }

    private boolean userHasThisWeeklyScore(AuthUser authUser, int weeklyScoreId){
        WeeklyScore weeklyScore = weeklyScoreRepository.findById(weeklyScoreId).orElse(null);
        return weeklyScore!=null && weeklyScore.getDashboard().getStudent().getId().equals(authUser.getUser().getId());
    }
    ...
    public boolean accessDifficultQuestion(AuthUser authUser, DifficultQuestion difficultQuestion){
        DifficultQuestion difficultQuestion1 = difficultQuestionRepository.findById(difficultQuestion.getId()).orElse(null);
        return difficultQuestion1!=null && difficultQuestion1.getDashboard().getStudent().getId().equals(authUser.getUser().getId());
    }
}