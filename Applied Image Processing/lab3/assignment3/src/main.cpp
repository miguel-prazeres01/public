...
static const SceneParams SceneParams_Art = {
    105,
    35,
    0,
    28,
...
int main()
{
    ...
    const auto input_select = InputSelection::Art;
    ...
    switch (input_select) {
        ...
        case InputSelection::Middlebury:
            ...
            break;
        case InputSelection::Art:
            // The Middleburry Stereo dataset.
            // Find plenty more in: https://vision.middlebury.edu/stereo/data/scenes2005/HalfSize/zip-2views/
            // Note that we use the half-sized images for faster testing.
            image = ImageRGB(dataDirPath / "art/view1.png");
            src_disparity = loadDisparity(dataDirPath / "art/disp1.png", 0.5f);
            scene_params = SceneParams_Art;
            break;
        default:
    ...
    plotGridMesh(dst_grid_rotate, { image.width * scene_params.grid_viz_im_scale, image.height * scene_params.grid_viz_im_scale }, scene_params.grid_viz_im_scale * scene_params.grid_viz_tri_scale).writeToFile(outDirPath / "11_rotated_grid.png", 1.0f, im_write_noise_level); 
    ...
    /* for (const auto& entry : std::filesystem::directory_iterator(outDirPath)) {
        ...
        ImageRGB output(entry.path().string());
        ImageRGB expected(dataDirPath / "expected/outputs" / entry.path().filename());

        assert(expected.width == output.width);
        assert(expected.height == output.height);

        for (int i = 0; i < output.height; i++) {
            for (int j = 0; j < output.width; j++) {
                auto expected_pixel = expected.data[i * expected.width + j];
                auto output_pixel = output.data[i * output.width + j];

                if (expected_pixel != output_pixel) {
                    std::cout << "----------------------Pixel at (" << j << ", " << i << ") is different!" << std::endl;
                    std::cout << "Expected: " << expected_pixel.r << ", " << expected_pixel.g << ", " << expected_pixel.b << std::endl;
                    std::cout << "Output: " << output_pixel.r << ", " << output_pixel.g << ", " << output_pixel.b << std::endl;
                }
            }
        }
        }*/
